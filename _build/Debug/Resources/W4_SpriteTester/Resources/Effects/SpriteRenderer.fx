...
void CreateVertex(inout TriangleStream<GS_DATA> triStream, float3 pos, float4 col, float2 texCoord, float rotation, float2 rotCosSin, float2 offset, float2 pivotOffset)
{
    ...
    {
		//Step 3.
        float2 originPos = pos.xy - offset - pivotOffset;
...
void MainGS(point VS_DATA vertex[1], inout TriangleStream<GS_DATA> triStream)
{
	...
	// LB----------RB
     ...
     // Calculate vertex positions based on pivot, scale, and rotation
    float2 pivotOffset = -pivot * gTextureSize * scale;
    float2 rotCosSin = float2(cos(rotation), sin(rotation));
    
    //VERTEX 1 [LT]
    position.xy = offset + float2(0, 0) * gTextureSize * scale;
    texCoord = float2(0, 0);
    CreateVertex(triStream, position, float4(1, 1, 1, 1), texCoord, rotation, rotCosSin, offset, pivotOffset); //Change the color data too!

	//VERTEX 2 [RT]
    position.xy = offset + float2(1, 0) * gTextureSize * scale;
    texCoord = float2(1, 0);
    CreateVertex(triStream, position, float4(1, 1, 1, 1), texCoord, rotation, rotCosSin, offset, pivotOffset); //Change the color data too!

	//VERTEX 3 [LB]
    position.xy = offset + float2(0, 1) * gTextureSize * scale;
    texCoord = float2(0, 1);
    CreateVertex(triStream, position, float4(1, 1, 1, 1), texCoord, rotation, rotCosSin, offset, pivotOffset); //Change the color data too!

	//VERTEX 4 [RB]
    position.xy = offset + float2(1, 1) * gTextureSize * scale;
    texCoord = float2(1, 1);
    CreateVertex(triStream, position, float4(1, 1, 1, 1), texCoord, rotation, rotCosSin, offset, pivotOffset); //Change the color data too!
}
