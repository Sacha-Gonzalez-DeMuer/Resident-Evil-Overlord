...
//PointLight
LightingResult DoPointLighting(Light light, Material mat, float3 V, float3 N, float3 P, float shadow)
{
	LightingResult result;
    ...
    result.Diffuse = DoDiffuse(light, L, N) * attenuation * light.Intensity * shadow;
    result.Specular = DoSpecular(light, mat, L, V, N) * attenuation * light.Intensity * shadow;
...
LightingResult DoSpotLighting(Light light, Material mat, float3 V, float3 N, float3 P)
{
    ...
    result.Specular = DoSpecular(light, mat, L, V, N) * attenuation   * light.Intensity;
