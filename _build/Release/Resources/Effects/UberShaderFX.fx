...
{
    float3 specular_blinn;
	
    if (gUseTextureSpecularIntensity)
    {
        float3 texSpecular = gTextureSpecularIntensity.Sample(gTextureSampler, texCoord);
        float specularIntensity = dot(texSpecular, float3(0.299, 0.587, 0.114));
        specular_blinn = gColorSpecular * pow(max(0.0f, dot(normalize(-gLightDirection + viewDirection), normal)), gShininess) * specularIntensity;
    }
    else
    {
        float3 half_vec = normalize(-gLightDirection + viewDirection);
        float blinn_term = dot(normal, half_vec);
		
        specular_blinn = gColorSpecular * pow(max(0.0f, blinn_term), gShininess);
    }
		
    return specular_blinn;
}

float3 CalculateSpecularPhong(float3 viewDirection, float3 normal, float2 texCoord)
{
    float3 specular_phong;
	
    if (gUseTextureSpecularIntensity)
    {
        float3 texSpecular = gTextureSpecularIntensity.Sample(gTextureSampler, texCoord);
        float specularIntensity = dot(texSpecular, float3(0.299, 0.587, 0.114));
        specular_phong = gColorSpecular * pow(max(0.0f, dot(reflect(-gLightDirection, normal), viewDirection)), gShininess) * specularIntensity;
    }
    else
    {
        float3 reflected_dir = reflect(-gLightDirection, normal);
        float specular_term = pow(max(dot(reflected_dir, viewDirection), 0.0f), gShininess);
		
        specular_phong = gColorSpecular * specular_term;
    }
	
    return specular_phong;
}
...
float3 CalculateSpecular(float3 viewDirection, float3 normal, float2 texCoord)
{
    ...
    float3 specColor = gColorSpecular * specular_term;

    if (gUseTextureSpecularIntensity)
    {
        float3 texSpecular = gTextureSpecularIntensity.Sample(gTextureSampler, texCoord);
        specColor *= texSpecular;
    }
...
float3 CalculateNormal(float3 tangent, float3 normal, float2 texCoord)
{
	...
	if(gUseTextureNormal)
    {
        ...
        float3x3 tangent_space = float3x3(tangent, binormal, normal);
		
        float3 normal_sample = gTextureNormal.Sample(gTextureSampler, texCoord);
		
        normal_sample = (2.0f * normal_sample) - float3(1.0f, 1.0f, 1.0f);
		
        return normalize(mul(normal_sample, tangent_space));
    }
...
float3 CalculateDiffuse(float3 normal, float2 texCoord)
{
    ...
    float cosA = max(0, dot(normal, -gLightDirection));
	
    return diffColor * cosA;
}
...
float3 CalculateFresnelFalloff(float3 normal, float3 viewDirection, float3 environmentColor)
{
    ...
    float fresnelTerm = pow(gFresnelMultiplier * (1.0 - dot(normal, viewDirection)), -gFresnelPower);

    if (gUseEnvironmentMapping)
    {
        return fresnelTerm * environmentColor;
    }
    return fresnelTerm * gColorFresnel.rgb;
}
...
float3 CalculateEnvironment(float3 viewDirection, float3 normal)
{
    ...
    if (gUseEnvironmentMapping)
    {
        ...
        environment = gCubeEnvironment.Sample(gTextureSampler, reflection);
		
        if (gReflectionStrength > 0.0f)
            environment.rgb = lerp(environment.rgb, float3(1.0f, 1.0f, 1.0f), gReflectionStrength);
	
        if (gRefractionStrength > 0.0f)
        {
            float3 refraction = refract(viewDirection, normal, gRefractionIndex);
            float3 refraction_color = gCubeEnvironment.Sample(gTextureSampler, refraction);
			
            environment.rgb = lerp(environment.rgb, refraction_color.rgb, gRefractionStrength);
        }
    }
...
float4 MainPS(VS_Output input) : SV_TARGET {
	...
	float3 specColor;
		...
		specColor=CalculateSpecularBlinn(viewDirection, newNormal, input.TexCoord);
	else if(gUseSpecularPhong)
		specColor=CalculateSpecularPhong(viewDirection, newNormal, input.TexCoord);
	else
        specColor = CalculateSpecular(viewDirection, newNormal, input.TexCoord);
