...
void ShadowMapRenderer::Initialize()
{
	...
	// create a separate rendertarget
	...
	auto result = m_pShadowRenderTarget->Create(desc);
	if (result != S_OK)
	{
		Logger::LogError(L"ShadowMapRenderer::Initialize() > Failed to create shadow render target");
		return;
	}
	...
	UINT staticType = static_cast<UINT>(ShadowGeneratorType::Static);
	UINT skinnedType = static_cast<UINT>(ShadowGeneratorType::Skinned);
...
void ShadowMapRenderer::UpdateMeshFilter(const SceneContext& sceneContext, MeshFilter* pMeshFilter) const
{
	...
	pMeshFilter->BuildVertexBuffer(sceneContext,
		techniqueContext.inputLayoutID, 
		techniqueContext.inputLayoutSize, 
		techniqueContext.pInputLayoutDescriptions);
...
void ShadowMapRenderer::DrawMesh(const SceneContext& sceneContext, MeshFilter* pMeshFilter, const XMFLOAT4X4& meshWorld, const std::vector<XMFLOAT4X4>& /*meshBones*/)
{
	...
	// 1.
	const ShadowGeneratorType shadowGenType{ pMeshFilter->HasAnimations() ? ShadowGeneratorType::Skinned : ShadowGeneratorType::Static};

	// 2.
	const auto& techniqueContext = m_GeneratorTechniqueContexts[static_cast<UINT>(shadowGenType)];
	...
	for (const auto& subMesh : pMeshFilter->GetMeshes())
	{
		...
		// Set correct TechniqueContext on ShadowMapMaterial - use ShadowGeneratorType as ID (BaseMaterial::SetTechnique)
		m_pShadowMapGenerator->SetTechnique(static_cast<UINT>(shadowGenType));
